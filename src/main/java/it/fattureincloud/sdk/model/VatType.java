  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    ...
    for (Entry<String, JsonElement> entry : entries) {
      if (!VatType.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `VatType` properties."
                    + " JSON: %s",
    ...
    if (jsonObj.get("ei_type") != null && !jsonObj.get("ei_type").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `ei_type` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("ei_type").toString()));
    }
    if (jsonObj.get("ei_description") != null && !jsonObj.get("ei_description").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `ei_description` to be a primitive type in the JSON string but"
                  + " got `%s`",
