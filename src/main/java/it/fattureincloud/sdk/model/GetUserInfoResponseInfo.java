...
public class GetUserInfoResponseInfo implements Serializable {
  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    if (jsonObj == null) {
      if (!GetUserInfoResponseInfo.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON object is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in GetUserInfoResponseInfo is not found in the empty JSON"
                    + " string",
    ...
    for (Entry<String, JsonElement> entry : entries) {
      if (!GetUserInfoResponseInfo.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `GetUserInfoResponseInfo`"
                    + " properties. JSON: %s",
