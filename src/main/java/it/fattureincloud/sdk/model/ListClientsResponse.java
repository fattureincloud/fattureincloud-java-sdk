...
public class ListClientsResponse implements Serializable {
  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    if (jsonObj == null) {
      if (!ListClientsResponse.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON object is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in ListClientsResponse is not found in the empty JSON"
                    + " string",
    ...
    for (Entry<String, JsonElement> entry : entries) {
      if (!ListClientsResponse.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `ListClientsResponse`"
                    + " properties. JSON: %s",
