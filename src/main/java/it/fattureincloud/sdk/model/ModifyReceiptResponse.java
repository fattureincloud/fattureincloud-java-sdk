...
public class ModifyReceiptResponse implements Serializable {
  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    if (jsonObj == null) {
      if (!ModifyReceiptResponse.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON object is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in ModifyReceiptResponse is not found in the empty JSON"
                    + " string",
    ...
    for (Entry<String, JsonElement> entry : entries) {
      if (!ModifyReceiptResponse.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `ModifyReceiptResponse`"
                    + " properties. JSON: %s",
